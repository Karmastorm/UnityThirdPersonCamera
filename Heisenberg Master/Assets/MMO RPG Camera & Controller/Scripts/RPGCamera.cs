...
public class RPGCamera : MonoBehaviour {
	...
	public float AlignCharacterSpeed = 180.0f;
	...
	private void LateUpdate() {
		...
		if (AlignCharacter == AlignCharacter.Always) {
			...
			if (Input.GetButtonDown(AlignCharacterInput)) {
				if (!_characterAligned && !_turningRoutineStarted) {
					StartCoroutine(TurningCoroutine());
					_characterAligned = true;
				}
	...
	public IEnumerator TurningCoroutine() {
		_turningRoutineStarted = true;

		Vector3 targetRotation = new Vector3(transform.eulerAngles.x, UsedCamera.transform.eulerAngles.y, transform.eulerAngles.z);
		
		while (Quaternion.Angle(transform.rotation, Quaternion.Euler(targetRotation)) > 1.0f) {
			// The angle on the Y-axis between the view direction of the character and the view direction of the camera is greater than 1 degree
			Quaternion before = transform.rotation;
			
			// Rotate the character towards the view direction of the camera
			transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(targetRotation), AlignCharacterSpeed);

			// Counter the accompanied camera rotation
			float delta = Quaternion.Angle(before, transform.rotation);
			float offsetToCameraRotation = CustomModulo(_mouseX, 360.0f);
