...
public class RPGMotor : MonoBehaviour {
	...
	public void StartMotor() {
		...
		if (_characterController.isGrounded) {
			...
			if (_jump) {
				...
				if (!_sliding & !_isStunned) {
					// Only jump if we are not sliding
					_playerDirectionWorld.y = JumpHeight;
				}
			}

		} else if (_allowAirborneMovement && !_runningJump && !_isStunned) {
	...
	private void ApplySliding() {
		...
		//Debug.DrawRay(transform.position + Vector3.up, Vector3.down * 1.4f, Color.magenta);
		if (Physics.Raycast(transform.position + Vector3.up, Vector3.down, out hitInfo, 1.4f)) {
